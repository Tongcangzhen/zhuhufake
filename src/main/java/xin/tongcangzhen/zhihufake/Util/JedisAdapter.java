...
public class JedisAdapter implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
         pool = new JedisPool("tongcangzhen.xin",46379);
    ...
    public Transaction multiTran(Jedis jedis) {
        try {
            return jedis.multi();
        } catch (Exception e) {
            logger.error("发生异常" + e.getMessage());
    ...
    public List<Object> execTrans(Transaction transaction, Jedis jedis) {
        try {
    ...
    public Double zscore(String key,String mem) {
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
            return jedis.zscore(key,mem);
        } catch (Exception e) {
            logger.error("发生异常" + e.getMessage());
        } finally {
            if (jedis != null) {
